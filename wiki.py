...
class Wikipedia:
    ...
    def __init__(self):
        ...
        self.foodon_pairs_file = 'data/FoodOn/pairsfile.txt'
    ...
    def parse_wiki(self):   # tested
        ...
        processed_labels = fdc_preprocess.preprocess_columns(pd.Series(labels), load_phrase_model=False, generate_phrase=False).tolist()
        queries = []
        for label in labels:
    ...
    def query_summary(self, queries, summary_file=None, failed_query_file=None):    # tested
        if summary_file and failed_query_file:
            ...
            print(f'Found {len(depracated_queries)} deprecated queries')
            print(f'Found {len(queries)} new queries')
