...
class WordEmbeddings:
    def __init__(self):
        ...
        self.summary_preprocessed_file = 'data/wikipedia/preprocessed.txt'
    ...
    def get_train_paras(self):
        paras = []
        files = os.listdir('data/corpus')
        for file in files:
            list_ = json.load(open('data/corpus/'+file, 'r', encoding='utf8'))
            fdc_preprocess = FDCPreprocess()
            list_ = fdc_preprocess.preprocess_columns(pd.Series(list_), load_phrase_model=False, generate_phrase=False)
            paras.extend(list_)
        return paras
    ...
    def train_embeddings(self):
        ...
        sents.extend(self.get_train_paras())
...
class Word2vecTrainer:
    def __init__(self):
        ...
        self.min_count = 1
        self.workers = 16
        self.vectors_save_file = 'data/model/word2vec_trained_without.txt'
    ...
    def train(self, sentences, pretrained_file=None):   # retraining of pretrained vectors is not possible in gensim 4 word2vec model.
        ...
        if pretrained_file:
            ...
            print(f'Intersecting {len(common_words)} common vocabularies for transfer learning')
            ...
            self.model.wv.intersect_word2vec_format(pretrained_file, lockf=1) # intersecting word vectors are replaced by pretrained vectors. lockf=1 means allow imported vectors to be trained. lockf=0 imported vectors are not trained.
