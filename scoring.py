...
class Scoring:
    ...
    def run_config(self):
        ...
        for alpha in [x*.1 for x in range(0, 11)]:
            for beta in [.1]:
                self.reset_nodes()
                self.alpha = alpha
                self.beta = beta
    ...
    def find_worst_classes(self):
        ...
        for _, node in self.class_dict.items():
            if len(node.all_entities) < 10:
                continue
            prec = 1 - (len(set(node.all_entities)-set(node.predicted_entities+node.seed_entities)) / len(set(node.all_entities)-set(node.seed_entities)))
            # if prec < .99:
            print(f'\nclass with {prec:.3f} precision class: {node.raw_label}')
            print('seeds: ', end=' ')
            for entity in node.seed_entities:
    ...
    def find_per_class_prec(self):

        self.find_precision_of_class(self.root)

        return None

    def find_precision_of_class(self, root):

        print(f'Class: {root.raw_label}, Total entities:{len(root.all_entities)}, predicted entities:{len(root.predicted_entities)}', end='')
        count = len(set(root.predicted_entities).intersection(set(root.all_entities)))
        print(f' correctly predicted: {count}, precision: {count/(len(root.all_entities)+1e-5):.2f}')

        if len(root.children) == 0: # leaf class
            return None

        for child in root.children:
            self.find_precision_of_class(child)
